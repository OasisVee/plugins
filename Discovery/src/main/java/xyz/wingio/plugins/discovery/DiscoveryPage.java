...
public final class DiscoveryPage extends SettingsPage {
	...
	public void onViewBound(View view) {
		...
		try {
			ReflectUtils.invokeMethod(loadMore, "setProgressBarColor", ColorCompat.getThemedColor(context, R.b.colorOnPrimary));
		} catch (Throwable e) {}
	...
	public DiscoveryResult loadMore(List<DiscoveryGuild> current, int limit) throws Throwable {
		String categoryParam = currentCategory != 0 ? "&categories=" + currentCategory : "";
		DiscoveryResult res = (DiscoveryResult) Http.Request.newDiscordRequest(
				String.format("/discoverable-guilds?limit=%s&offset=%s%s", limit, current.size(), categoryParam),
				"GET")
	...
	public DiscoveryResult loadMore(List<DiscoveryGuild> current, int limit, String query) throws Throwable {
		String categoryParam = currentCategory != 0 ? "&categories=" + currentCategory : "";
		DiscoveryResult res = (DiscoveryResult) Http.Request.newDiscordRequest(
				String.format("/discoverable-guilds?limit=%s&offset=%s&query=%s%s", limit, current.size(), query, categoryParam),
				"GET")
			.setHeader("Referer", "https://discord.com/guild-discovery")
			.execute()
			.json(DiscoveryResult.class);
	...
	public void configureCategories(ChipGroup categories) {
		...
		categories.setChipSpacingVertical(p / 4);
		categories.setChipSpacingHorizontal(p / 4);
			...
			if (!text.isEmpty()) {
				...
				Utils.threadPool.execute(() -> {
					try {
						...
						page.logger.error("Search failed", e);
						Utils.mainThread.post(() -> {
							loading.setVisibility(View.GONE);
							page.setActionBarSubtitle("Search failed");
							adapter.setData(page.cache);
							loadMore.setVisibility(View.VISIBLE);
						});
					}
				});
				...
				search.getRoot().setEndIconOnClickListener(s -> {
					adapter.setData(page.cache);
					page.setActionBarSubtitle(page.plugin.totalDiscoveryServers + " servers");
